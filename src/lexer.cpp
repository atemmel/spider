...
Next:
	...
	if (*it == '#') {
		...
		it =
		    std::find_if(it, end, [](const char c) { return std::isspace(c); });
		...
		// If token
		if (auto vt = std::find(validTokens.begin(), validTokens.end(), word);
		    vt != validTokens.end()) {
			token.type = static_cast<Token::Type>(
			    std::distance(validTokens.begin(), vt));
		}
		// If setting
		else if (auto vc =
		             std::find(validConfig.begin(), validConfig.end(), word);
		         vc != validConfig.end()) {
			int offset = static_cast<int>(Token::Type::ConfigOffset) + 1;
			token.type = static_cast<Token::Type>(
			    offset + std::distance(validConfig.begin(), vc));
		}
